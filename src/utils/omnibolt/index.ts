...
import { parseOmniboltUri } from 'omnibolt-js/lib/utils';
import {
...
/**
 * This parses connect information generated by getConnectUri.
 * @async
 * @param data
...
export const parseOmniboltConnectData = async (
	data = '',
): Promise<Result<IParseOmniboltUriResponse>> => {
	try {
		data = data.trim();
		const parsedData = parseOmniboltUri(data);
		if (parsedData.isErr() || parsedData?.value?.action !== 'connect') {
			return err('Invalid Omnibolt Connect Data');
		}
		return ok(parsedData.value);
	} catch (e) {
		return err(e);
	}
};
 ...
 * @return {string}
 */
export const getConnectUri = (): string => {
	const uri = obdapi.getConnectUri();
	if (uri.isErr()) {
		return '';
	}
	return uri.value;
...
export const createAsset = async ({
	...
	try {
		const address = await obdapi.getFundingAddress({});
		if (address.isErr()) {
			return err(address.error.message);
		}
		const info: IssueFixedAmountInfo = {
			from_address: address.value.address,
			name,
			ecosystem: 2,
			divisible_type: 2,
...
export const connectAndOpenChannel = async ({
	...
	try {
		...
		// Retrieve the omnibolt funding address.
		...
		//Ensure the funding address has enough of the specified omni asset to fund the channel the given amount.
		const omniAssetBalanceResponse = await obdapi.getAllBalancesForAddress(
			fundingAddress.value.address,
		);
		if (omniAssetBalanceResponse.isErr()) {
			return err(omniAssetBalanceResponse.error.message);
		}
		//Grab balance of the specified asset id.
		const omniAssetBalance = omniAssetBalanceResponse.value.filter(
			(asset) => asset?.propertyid === info?.info?.asset_id,
		);
		//Asset id not found for the given address.
		if (omniAssetBalance?.length <= 0) {
			return err(
				`The provided funding address does not possess the specified Omni asset (${info.info.asset_id})`,
			);
		}
		//Asset id balance not larger enough to fund the channel.
		if (Number(omniAssetBalance[0]?.balance) < Number(info.info.asset_amount)) {
			const difference =
				Number(info.info.asset_amount) - Number(omniAssetBalance[0]?.balance);
			return err(
				`The provided funding address needs ${difference} more of asset ${info.info.asset_id} in order to open a channel. `,
			);
		}
		// Ensure the funding address has enough BTC to fund the channel 3 times.
		const fundingAddressBalance = await getAddressBalance({
			addresses: [fundingAddress.value.address],
			selectedNetwork,
		});
		...
		if (fundingAddressBalance.isErr()) {
			return err(fundingAddressBalance.error.message);
		}

		/*
		 * TODO: Find a better way to determine the satMinimum based on current fee estimates from the curent obd node.
		 */
		const satMinimum = 60000;

		// Check that we even have enough sats to open and fund a channel.
		if (
			fundingAddressBalance.value < satMinimum &&
			onChainBalance < satMinimum
		) {
			return err('Not enough sats to create, fund and open a channel');
		}

		// Attempt to fund the address if it's lower than the satMinimum.
		//TODO: We'll need to prompt the user before sending BTC to the funding address.
		if (fundingAddressBalance.value < satMinimum) {
			const setupResponse = await setupOnChainTransaction({
				selectedNetwork,
				selectedWallet,
			...
			await updateOnChainTransaction({
				...
				transaction: {
					...
					outputs: [
						{
							address: fundingAddress.value.address,
							value: satMinimum,
							index: 0,
						},
					],
			...
			if (broadcastRes.isErr()) {
				return err(broadcastRes.error.message);
			}
			// Refresh wallet and then proceed to fund the temp channel.
			await refreshWallet();
		}

		// Attempt to create a temp channel with the specified node.
		return await obdapi.createChannel(info);
	} catch (e) {
		return err(e);
	}
};
...
/**
 * Used to independently continue the funding process in the event an error was encountered for a given temp channel.
 * @param {string} tempChannelId
 * @param {TAvailableNetworks} [selectedNetwork]
 * @param {string} [selectedWallet]
 * @param {string} nodePeerId
 * @param {string} userPeerId
 * @param {string} nodeAddress
 */
export const fundTempChannel = async ({
	tempChannelId,
	selectedNetwork,
	selectedWallet,
	nodePeerId,
	...
	// Ensure we're connected to the remote_node_address.
	await obdapi.connectPeer({
		remote_node_address: nodeAddress,
	});
	...
	const tempChannels =
		getStore().omnibolt.wallets[selectedWallet].tempChannels[selectedNetwork];
	const tempChannel = tempChannels[tempChannelId];
	const timesFunded = tempChannel.btc_funding_times;
	if (timesFunded === 3) {
		return err('Already funded channel 3 times.');
	}
	const fundTempChannelResponse = await obdapi.fundTempChannel({
		recipient_node_peer_id: nodePeerId,
	...
	if (fundTempChannelResponse.isErr()) {
		return err(fundTempChannelResponse.error.message);
	}
	await updateOmniboltChannels({ selectedWallet, selectedNetwork });
	return ok(fundTempChannelResponse.value);
};
